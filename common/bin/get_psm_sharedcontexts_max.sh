...
processes_per_context="4"
#AH-Qlogic 20120329 Workaround for intranode MPI run. Should be fixed by next Qlogic driver release.
#Check for intranode computation and set PSM_DEVICE if required
if [ "$OMPI_COMM_WORLD_LOCAL_SIZE" == "$OMPI_COMM_WORLD_SIZE" ]; then
