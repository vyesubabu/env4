...
## v3.3b2-rev3 :Modification Date: 2016/04/08 by RM : 
#		-management of dirname in filesystem name 
#
## v3.3b2-rev2 :Modification Date: 2016/03/18 by RM : 
#		- change PrdDB filename  to avoid 20160318.key1.key2.key3 appearing..
#
## v3.3b2-rev1 :Modification Date: 2015/11/19 by RM/MSP : 
#		- use config Parser
#		- option _originalExtension for sending without .bin 
## v3.3b2 :Modification Date: 2015/11/17 by RM : 
#		- Reuse TRSJAN ip & config
# v3.3b1 :Modification Date: 2015/11/03 by RM : 
#		- bugfix: date position as per CB/SBL definition AFTER the 3 keys
#		- bugfix: create cips_send.log dir automatically
# v3.3b1 :Modification Date: 2015/10/30 by RM : 
#		- bugfix: 
# v3.2.0-rev1 :Modification Date: 2014/09/02 by RM : 
#		- bugfix: .nc.bmkg.local (instead of .nc.nmkg.local..)
# v3.2b2-rev3 :Modification Date: 2014/08/27 by RM : 
#		- Add: .nc.bmkg.local
#		- Mod: TRSJAN on 172.19.22.69
# v3.2b2-rev2 :Modification Date: 2014/03/19 by RM : 
#		- Add: .mfi.local to both cips servers (not seen on compute..)
# v3.2b2-rev1 :Modification Date: 2014/03/17 by RM : 
#		- Add: findParentBinary function when failing to find files
#		- Mod: change ip of TRANSMET to 10.0.4.69
...
## Activate the keeping of the original extension for sending to Transmet with 1
_originalExtension = 1
## Activate the renaming to .bin with 0
#_originalExtension = 0

## Start Imports & functions
import ConfigParser
import gettext, datetime, os, sys
from string import upper, replace

Config = ConfigParser.ConfigParser()

#CIPS_TC_CONFIGFILE = "/cm/shared/client_config/v3.3/BMKG.v3.3.conf"
#CIPS_TC_CONFIGFILE = "/cm/shared/client_config/v3.3/MiniHPCBotswana.MFI.v3.3b2.conf"
CIPS_TC_CONFIGFILE = "/ecfshared/v4.0/MainConfig/ecFlow-Central.MFI.v4.0b1.conf"

Config.read(CIPS_TC_CONFIGFILE)

def getConfig(config, section, option):
    temp_list = []
    temp_val = ''

    temp_section = Config.sections()
    #print temp_section
    #print 'Cips' in temp_section

    if section in temp_section:
        temp_option = Config.options(section)
        #print Config.options(section)

        if option.lower() in temp_option:
            temp = config.get(section, option)

            if temp.find(',') > -1:
                temp_list = temp.split(',')
                for _list in temp_list:
                    temp_val += _list + ', '
            else:
                temp_val = temp
            return temp_val
        else:
	    print "option %s not defined:" % option
            sys.exit(1)
            #return 'There is no option %s' % option
    else:
        #return 'There is no section %s' % section
	print "Section %s not defined:" % section
        sys.exit(1)
...
#[Cips]
### RM update 
CIPSSTANDBY = getConfig(Config, 'Cips', 'CIPSSTANDBY')
CIPSOPER    = getConfig(Config, 'Cips', 'CIPSOPER')
CIPS_HOSTS = [CIPSSTANDBY, CIPSOPER]
CIPS_LOGIN = getConfig(Config, 'Cips', 'CIPS_LOGIN') 
CIPS_PASSW = getConfig(Config, 'Cips', 'CIPS_PASSW')

#[Transmet]
## TRSJAN (not in DNS)
TRANSMET    = getConfig(Config, 'Transmet', 'TRANSMET_IP')
TRANSMET_HOSTS = [TRANSMET]
TRANSMET_LOGIN = getConfig(Config, 'Transmet', 'TRANSMET_LOGIN')
TRANSMET_PASSW = getConfig(Config, 'Transmet', 'TRANSMET_PASSW')
TRANSMET_PATH  = getConfig(Config, 'Transmet', 'TRANSMET_PATH')
...
TRANSMET_HEADERS[CST_LRFDB] = "ZCLR00"
...
TRANSMET_CENTERS[CST_LRFDB] = "CIPS"
...
"""
Figure out parent process id & binary file
"""
def findParentBinary():
   import socket, posix
   hostname = socket.gethostname()
   ppid = str(os.getppid())
   print "My caller's id = " + ppid
   stringcmd = "file /proc/"+ppid+"/exe"
   #print stringcmd
   os.system(stringcmd)
   print "Launching: "+stringcmd
   p = os.popen(stringcmd,"r")
   while 1:
      line = p.readline()
      if not line: break
      binary = line.split()[4].replace("`","").replace("'","")
      print "Parent's binary is "+binary
      return "Called from : " + binary + " on host " + str(hostname)
...
def generatePrdDBFilename(file, keys, date=None):
  ...
  ## do NOT handle dirname at this level
  keyPrefix = str(date)+"."+keys['key1']+"."+keys['key2']+"."+keys['key3']+"."+filename


#  print ("Using filename for PrdDB ", keyPrefix)
## Modif du 18/03/2016 durant le debug GFS  (doesnt work)
  #keyPrefix = filename

  return keyPrefix
  
"""
Make symbolic link for a file with correct LrfDB syntax:
  YYYYMMDD.key1.key2.key3.originalFileName

@input  file      Original filename
@input  keys      [key1, key2, key3]
@input  date      Date for LrfDB
@output fileName   LrfDB filename
"""
def generateLrfDBFilename(file, keys, date=None):
  filename = os.path.basename(file)
  todayDate = datetime.date.today().strftime("%Y%m%d")
  ...
  ## Commented by RM on 20170227 : coherent with Transmet naming
  #keyPrefix = keys['key1']+"."+keys['key2']+"."+keys['key3']+"."+str(date)+"."+filename
  keyPrefix = keys['key1']+"_"+keys['key2']+"_"+keys['key3']+"_"+str(date)+"_"+filename
...
def generateTransmetFilename(header, center, freeformat, date=None):
  ...
  allbutlast = npoints -1
  extension = os.path.splitext(freeformat)[1]
  freeformat_allbutlast = replace(freeformat, ".", "_",allbutlast)
  freeformat_all = replace(freeformat, ".", "_")
  
  # Add ProcessId to filename
  pid = str(os.getpid())
  
  # Rename for Transmet 
  if (_originalExtension):
    fileName = "T_"+header+"_C_"+center+"_"+date+"_"+freeformat_allbutlast
  else:
...
def usage():
  ...
  print "                             Could be: moddb, imgdb, prddb, lrfdb"
  ...
  print " "
  print "   LrfDB with specific keys:"
  print "       --date YYYYMMDD           Specify date (optional)"
  print "       --key1 Model              Specify Model"
  print "       --key2 Type               Specify Type"
  print "       --key3 SubType            Specify SubType"
  print ""  
  ...
  ##########################
    ...
    for file in files:
      ...
      if (args['database'] in (CST_PRDDB)):
        freeformatFile = generatePrdDBFilename(file, args['prddb'], args['prddb']['date'])
      elif (args['database'] in (CST_LRFDB)):
        freeformatFile = generateLrfDBFilename(file, args['lrfdb'], args['lrfdb']['date'])
      else:
        freeformatFile = file
  ...
  ##########################
    ...
    for file in files:
      ...
      if (args['database'] in (CST_PRDDB)):
        ...
        fileName = generateLrfDBFilename(file, args['lrfdb'], args['lrfdb']['date'])
      else:
        fileName = file
...
if __name__ == "__main__":
  ...
  ##########################
  if (not files):
    #print "RM wAs here:" ,os.getppid()
    error(_("--files argument required"))
    #usage()
    #print "RM was here" + sys.exc_info()
    figureOutHostandCmd = findParentBinary() 
 

    print figureOutHostandCmd
    error(_( figureOutHostandCmd ))
    #print "RM wAs here:" ,os.getppid()
    #error(_("Parent pid = " + str(os.getppid()) ))
    sys.exit(1)
  ...
  if (upper(database) == upper(CST_LRFDB) ):
    if (not key1):
      error(_("--key1 argument required for database LrfDB"))
      sys.exit(1)
    if (not key2):
  ...
  if (_log):
    ...
    print "Getting config infos from %s " % CIPS_TC_CONFIGFILE
    print "CIPS_S IP is %s " % CIPSSTANDBY
    print "TRANSMET IP is %s " % TRANSMET
    # Fill log file
